        ...
        for key, w in zip(col_keys, weights):
            ...
            if key == '方向' or key == '买卖方式':
                ...
                try:
                    ...
                    pass
            lbl = Label(text=value, font_size=FS['sm'], color=txt_color, halign='right', size_hint_x=w)
            self.add_widget(lbl)
...
class WatchlistPanel(BoxLayout):
    ...
    def _add_row(self, sym, price, entry, direct, status_text):
        ...
        set_bg(row, bg)
        lbl_sym   = Label(text=sym, font_size=FS['sm'], color=C['text'], halign='center', size_hint_x=0.15)
        lbl_price = Label(text=f"{float(price):.2f}", font_size=FS['sm'], color=C['text'], halign='right', size_hint_x=0.18)
        lbl_entry = Label(text=f"{float(entry):.2f}", font_size=FS['sm'], color=C['text'], halign='right', size_hint_x=0.18)
        lbl_dir   = Label(text=direct, font_size=FS['sm'], color=C['buy'] if '买' in direct else C['sell'], halign='center', size_hint_x=0.24)
        lbl_st    = Label(text=status_text, font_size=FS['sm'], color=C['success'] if status_text=='运行中' else C['text_muted'], halign='center', size_hint_x=0.25)
...
class MonitorApp(App):
    def build(self):
        ...
        class TabButton(BoxLayout):
            def __init__(self, text,** kwargs):
                ...
                self.set_normal()
            def set_selected(self): set_bg(self, C['primary']); self.label.color = C['white']
            def set_normal(self): set_bg(self, C['bg_card']); self.label.color = C['text_muted']
        ...
        self.content.add_widget(self.account_panel)
    ...
    def on_connect_success(self, account, readonly=True):
        self.account_panel.update(account)
        self.watchlist_panel._load()

    def refresh_account(self): self.account_panel.update({'account_name':'同步账户'})
